...
void Network::forward(vector<double> input,vector<double> target)
{
    ...
    layers[num_layer - 1]->apply_loss_func(target, error, dErr_dy);
}
...
void Network::backward(vector<double> input) {
    ...
    if (num_layer == 1) {
        ...
        layers[num_layer - 1]->feed_backward(layers[num_layer-2]->output, dErr_dy);
        for (int i = num_layer - 2; 0 < i; i--) {
            layers[i]->feed_backward(layers[i - 1]->output, layers[i+1]->del);
        }
        layers[0]->feed_backward(input, layers[1]->del);
    }
}
...
void Network::write_all_weight()
{
    char filename[25]="";
    for (int i = 0; i < num_layer; i++) {
        sprintf(filename, "%2d-th_layer_w.txt", i);
